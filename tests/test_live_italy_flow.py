...
class LiveItalyFlowTests(unittest.TestCase):
    maxDiff = None
    ...
    def test_multiple_products_return_documents(self) -> None:
        for product, run in self.runs.items():
            with self.subTest(product=product):
                self.assertGreater(len(run.documents), 0, msg=f"{product} returned no documents for the Italy flow.")
    ...
    def test_relevant_implemented_sources_return_documents_across_suite(self) -> None:
        ...
        self.assertGreater(
            docs_by_source["aifa_italy"],
            0,
            msg="Implemented source 'aifa_italy' returned no documents across the Italy test products.",
        )

    def test_aifa_returns_document_or_page_outputs(self) -> None:
        for product, run in self.runs.items():
            with self.subTest(product=product):
                aifa_docs = [doc for doc in run.documents if doc.source_id == "aifa_italy"]
                self.assertGreater(len(aifa_docs), 0)
                self.assertTrue(any(doc.format in {"pdf", "zip", "html"} for doc in aifa_docs))
    ...
    def test_titles_or_urls_match_expected_brand_or_generic(self) -> None:
        for product, expected in [
            ...
            with self.subTest(product=product):
                docs = [doc for doc in self.runs[product].documents if doc.source_id == "aifa_italy"]
                self.assertGreater(len(docs), 0)
                self.assertTrue(
                    any(
                        expected in doc.title.lower()
                        or expected in doc.document_url.lower()
                        or expected in doc.page_url.lower()
                        for doc in docs
                    )
                )
