...
class LiveSpainFlowTests(unittest.TestCase):
    maxDiff = None
    ...
    def test_multiple_products_return_documents(self) -> None:
        for product, run in self.runs.items():
            with self.subTest(product=product):
                self.assertGreater(len(run.documents), 0, msg=f"{product} returned no documents for the Spain flow.")
    ...
    def test_relevant_implemented_sources_return_documents_across_suite(self) -> None:
        ...
        self.assertGreater(
            docs_by_source["aemps_spain"],
            0,
            msg="Implemented source 'aemps_spain' returned no documents across the Spain test products.",
        )

    def test_aemps_returns_pdf_or_html_outputs(self) -> None:
        for product, run in self.runs.items():
            with self.subTest(product=product):
                aemps_docs = [doc for doc in run.documents if doc.source_id == "aemps_spain"]
                self.assertGreater(len(aemps_docs), 0)
                self.assertTrue(any(doc.format in {"pdf", "html"} for doc in aemps_docs))
    ...
    def test_titles_or_urls_match_expected_brand_or_generic(self) -> None:
        for product, expected in [
            ...
            with self.subTest(product=product):
                docs = [doc for doc in self.runs[product].documents if doc.source_id == "aemps_spain"]
