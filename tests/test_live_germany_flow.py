...
class LiveGermanyFlowTests(unittest.TestCase):
    maxDiff = None

    @classmethod
    def setUpClass(cls) -> None:
        cls.products = ["Keytruda", "Opdivo", "Rinvoq"]
        cls.country = "Germany"
        cls.runs = {
    ...
    def test_multiple_products_return_documents(self) -> None:
        for product, run in self.runs.items():
            with self.subTest(product=product):
                self.assertGreater(len(run.documents), 0, msg=f"{product} returned no documents for the Germany flow.")
    ...
    def test_pdf_outputs_exist_for_gba_results(self) -> None:
        for product, run in self.runs.items():
            with self.subTest(product=product):
                gba_docs = [doc for doc in run.documents if doc.source_id == "gba_germany"]
                self.assertGreater(len(gba_docs), 0)
                self.assertTrue(any(doc.format == "pdf" for doc in gba_docs))
    ...
    def test_titles_or_urls_match_expected_brand_or_generic(self) -> None:
        for product, expected in [
            ...
            with self.subTest(product=product):
                docs = [doc for doc in self.runs[product].documents if doc.source_id == "gba_germany"]
